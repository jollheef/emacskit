...
add_binds("normal", {
    ...
    -- Follow link
    buf("^а$", function (w,b,m)
        w:start_follow(modes.normal, nil, function (sig) return sig end)
    end),

    -- Focus element
    buf("^жж$", function (w,b,m)
        w:start_follow(modes.focus, "focus", function (sig) return sig end)
    end),
