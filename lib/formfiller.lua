...
add_binds("normal", {
    ...
    buf("^яф$", function (w) w:formfiller("add")  end),
    buf("^ят$", function (w) w:formfiller("new")  end),
    buf("^яу$", function (w) w:formfiller("edit") end),
    buf("^яд$", function (w) w:formfiller("load") end),
})
...
add_binds("formfiller", lousy.util.table.join({
    ...
    key({}, "й", function (w) w:set_mode() end),
